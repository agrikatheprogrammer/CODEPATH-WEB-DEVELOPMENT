...
function App() {
  ...
  function handleNext() {
    if (count < myjson.length - 1) setCount(count + 1);
    else alert('You have reached the end of the quiz!');
    setGuess('');
    setFeedbackColor('');
  }
  ...
  function handleBack() {
    ...
    else alert('You are at the start of the quiz!');
    setGuess('');
    setFeedbackColor('');
  }

  function guesstheans(input) {
    return !!(input.toLowerCase().includes(myjson[count].answer.toLowerCase()));
  }

  function handleGuessSubmit() {
    if (guesstheans(guess)) {
  ...
  return (
    <div className='main'>
      ...
      <Card
        question={myjson[count].question}
        answer={myjson[count].answer}
        level={myjson[count].level}
        image={myjson[count].image}
      />

      <div className="guesstheans">
        <label htmlFor="guesstheans-input">Guess the answer:</label>
        <input
          id="guesstheans-input"
          name="guesstheans"
          type="text"
          value={guess}
          onChange={(e) => setGuess(e.target.value)}
          style={{ backgroundColor: feedbackColor || 'white' }}
        />
        <input
          id="guesstheans-btn"
          type="button"
